...
**Arguments:** `$ARGUMENTS` (required: `patch`, `minor`, `major`, or explicit version like `0.2.0`)
...
if [ "$CURRENT" != "main" ]; then
  echo "Error: Must be on main branch to release. Currently on: $CURRENT"
  exit 1
fi
...
4. **Consolidate related changes**: Group similar items
...
```bash
CURRENT=$(grep '^version' extension.toml | head -1 | sed 's/version = "\(.*\)"/\1/')
...
Insert the new entry after the header and before the first existing version entry.
...
echo ""
echo "GitHub Actions:"
echo "  - Release: https://github.com/ggfevans/zed-hujson/actions/workflows/release.yml"
echo ""
echo "Once complete, release will be at:"
echo "  https://github.com/ggfevans/zed-hujson/releases/tag/v$NEW_VERSION"
```
...
Monitor at: https://github.com/ggfevans/zed-hujson/actions
```
