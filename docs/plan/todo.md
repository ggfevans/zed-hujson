...
1. **Grammar location** → grammar lives in its own repo **[`ggfevans/tree-sitter-hujson`](https://github.com/ggfevans/tree-sitter-hujson)** (revised again 2026-05-06). `extension.toml`'s `[grammars.hujson]` references it by `repository` URL + `rev`, no `path` key. The earlier `grammars/hujson/` subfolder arrangement was abandoned because Zed's grammar loader expects the configured repository URL to be clone-able into the destination directory; the monorepo arrangement caused `zed: install dev extension` to fail. Extracting the grammar also matches the Zed registry convention and makes the grammar consumable by Neovim/Helix without going through the extension.
