...
1. **Grammar lives in its own repo `ggfevans/tree-sitter-hujson`** (revised again 2026-05-06; supersedes both the original "repo root" plan and the interim "`grammars/hujson/` subfolder" arrangement). The Zed extension only contains the manifest, language config, Zed-specific queries (`queries/hujson/`), and the Rust extension crate. The grammar — `grammar.js`, `package.json`, `tree-sitter.json`, `binding.gyp`, `bindings/`, `src/`, `test/`, and the grammar's own `queries/highlights.scm` — was extracted to a standalone repo. `extension.toml`'s `[grammars.hujson]` sets `repository = "https://github.com/ggfevans/tree-sitter-hujson"` and `rev = "<commit>"`, with no `path` key. This was forced by Zed's grammar loader, which expects each grammar's repository URL to be clone-able into the destination directory; the monorepo layout caused dev install to fail.

> **Reading the prompts below**: prompts 2–8 (grammar foundation, mutation, corpus suite) and prompt 14's `grammar-test` CI job now live in the `tree-sitter-hujson` repo, not here. Prompts that produce zed-hujson artefacts (manifest, language config, Zed queries, Rust crate, CI for build/lint, README, CHANGELOG) still apply to this repo.
