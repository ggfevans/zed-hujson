...
1. **Grammar at repo root** — no `grammars/hujson/` subfolder. `grammar.js`, `package.json`, `tree-sitter.json`, `binding.gyp`, `test/corpus/`, and the generated `src/parser.c` + `src/grammar.json` + `src/node-types.json` + `src/tree_sitter/*` all live at the top level. `extension.toml`'s `[grammars.hujson]` omits the `path` key. The grammar's standalone queries (Neovim/Helix consumers) live under `tree-sitter-queries/`; Zed's queries stay under `queries/hujson/`.
